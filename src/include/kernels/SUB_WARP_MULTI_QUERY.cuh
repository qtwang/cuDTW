...
template <
    ...
    typename index_t,
    typename value_t>
__global__ void sub_warp_DTW_multi_query(
    value_t *Subject,
    value_t *Dist,
    index_t num_entries,
...
)
{
    ...
    for (index_t query_number = 0; query_number < num_queries; query_number++)
    {
        ...
        if (thid % group_size == 0)
            query_value = new_query_value;
        if (thid % group_size == 0)
            penalty_here1 = 0; // (query_value - subject_value0)*(query_value - subject_value0);
        ...
        for (index_t k = 3; k < lane + group_size - 1; k++)
        {
            ...
            if (counter % group_size == 0)
                new_query_value = cQuery[query_number * num_features + i + 2 * (thid % group_size) - 1];
        ...
        if (thid % group_size == group_size - 1)
            Dist[query_number * num_entries + (32 / group_size) * blockIdx.x + thid / group_size] = penalty_here31;
    }
