...
int main(int argc, char *argv[])
{
    ...
    { // query CBF database
        ...
        std::cout << "We are going to process "
                  ...
                  << std::endl;
        const value_t DM = (num_entries * num_features + num_entries) * sizeof(value_t);
    ...
    { // query ECG stream
        const value_t CU = (num_entries - num_features + 1) * num_features * num_features;
        std::cout << "We are going to process "
                  << CU / 1000000000000.0
                  << " Tera Cell Updates (TCU)"
                  << std::endl;
        const value_t DM = (num_entries - num_features + 1) * 2 * sizeof(value_t);
